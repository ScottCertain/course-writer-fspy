"""
Integration tests for the DraftPipeline.

Migrated from the standalone test_draft_pipeline.py script.
"""
...
@pytest.mark.asyncio
async def test_draft_pipeline_integration(mock_llm_service, test_course_dir):
    """
    Integration test for the draft pipeline with a mocked LLM service.
    ...
    lesson_id = "test_integration_draft"
    ...
    with patch(
        ...
        # Verify that final draft steps are NOT present
        assert "intro_conclusion" not in step_names
        assert "final_lesson" not in step_names
...
@pytest.mark.asyncio
async def test_draft_pipeline_error_handling(mock_llm_service, test_course_dir):
    """
    Test draft pipeline error handling by simulating a failure in the LLM service.
    """
    # Create a test lesson
    lesson_id = "test_draft_error_handling"
