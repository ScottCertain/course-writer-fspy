...
class FixedAnthropicLLMService(LLMService):
    """Updated LLM service for Anthropic Claude models."""

    # Model maximum token map - updated with correct max tokens
    MODEL_MAX_TOKENS = {
    ...
    def __init__(
        ...
        # Using an older model name that's more likely to be recognized
        self.model = model
        self.base_url = "https://api.anthropic.com/v1/messages"
        self.logger.info(f"Initialized Fixed Anthropic LLM service with model: {model}")
    ...
    async def generate_text(
        ...
        # Updated headers with the latest Anthropic API version
        headers = {
    ...
    async def generate_with_context(
        ...
        # Updated format for context and prompt - using system parameter at the top level
        headers = {
            "Content-Type": "application/json",
            "x-api-key": self.api_key,
            "anthropic-version": "2023-06-01",
        ...
        # FIXED: Use the system parameter at the top level instead of as a message role
        data = {
            "model": self.model,
